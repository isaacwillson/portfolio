...
export const PILLARS: { title: string; body: string; evidence: string }[] = [
  {
    ...
    body: "When the numbers I needed did not exist anywhere, I transcribed a summer of paper sign-in sheets by hand rather than settle for a worse question.",
    evidence: "250 observed hours, 29 days, collected from scratch",
  },
  {
    title: "I ship the whole thing",
    body: "A model in a notebook is not finished. Mine trains offline, deploys to Lambda as a container image, and answers a live site my neighbors actually open.",
    evidence: "300+ visitors on the dashboard it feeds",
  },
  {
    title: "I report honestly",
    body: "Every result is quoted against a real baseline, including the time my margin narrowed as the season grew. A number you cannot check is not a result.",
...
export const PROJECTS: Project[] = [
  {
    ...
    body: [
      "The pool logged arrivals on paper sign-in sheets and nothing else. I transcribed a summer of them into a training table of 250 observed hours across 29 days, joined each hour to Open-Meteo weather, and trained a gradient-boosting regressor on the result.",
      "The interesting decision was which hours to keep. Days the pool was closed, and days whose sheet went missing, are excluded entirely, because we cannot say what happened during them. But days the pool was open and nobody came are kept as genuine zero-turnout hours, since dropping those would delete exactly the bad-weather signal the model exists to learn.",
  ...
  },
  {
    ...
    body: [
      "The sibling to the forecaster, and the one people actually use, with 300+ unique visitors so far. It answers a question the model deliberately cannot: not what turnout should look like, but what the pool is like right now.",
      ...
      "Most of the remaining work went into the states that are not the happy path. Crowd levels are only tracked on some days, so on an untracked day the dashboard says so plainly, names the next tracked day, and points at the typical pattern rather than showing a stale number or a fake zero. Residents check it on their phones on the way out the door, so every section was built for a narrow screen first.",
    ],
    metrics: [
  ...
  },
  {
    ...
    name: "Chess AI",
    period: "Jun 2026",
    tagline: "A chess engine that plays you, with difficulty that means something.",
    body: [
      ...
      "The three difficulty tiers are search depth, not artificial mistakes. Easy searches shallowly and misses tactics because it genuinely cannot see them, while hard searches deep enough to punish you for the same moves.",
    ],
    metrics: [
...
export const ABOUT = [
  "I am a computer science and data science double major at Rutgers, and in the summers I work at the pool this whole site keeps talking about.",
  "That job is where the projects came from. I was the one logging resident visits against a list of a thousand units, which meant I was also the one who could see that nobody, residents included, had any real idea how busy the pool got. So I built the dashboard, then went back through a summer of those paper sheets to train something that could predict it.",
  "It taught me more about evaluation, deployment, and writing things down clearly than any tutorial would have, mostly because there was nobody else to check the work.",
];
